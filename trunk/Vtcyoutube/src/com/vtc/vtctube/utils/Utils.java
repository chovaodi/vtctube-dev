...
public class Utils {
    ...
    public static void settingControlRemove(int widht, SwipeListView swipeListView, Activity activity) {
    ...
    public static void getDialogMessges(Activity activity, String mes) {
        ...
        dialog.getWindow().setBackgroundDrawableResource(R.drawable.background_card);
    ...
    public static List<ItemPost> checkLikeVideo(List<ItemPost> list, List<ItemPost> listVideoLike) {
    ...
    public static ArrayList<ItemPost> getVideoLike(String sql, int tabIndex) {
        ...
        try {
            Cursor c = MainActivity.myDbHelper.query(DatabaseHelper.TB_LIKE, null, null, null, null, null, null);
    ...
    public static ItemPost getItemPostRandom(JSONObject json) {
        ...
        try {
            ...
            String urlThumnail = json.getJSONObject("thumbnail_images").getJSONObject("full").getString("url");
    ...
    public static ItemPost getItemPost(JSONObject json, int pageCount, int tabIndex) {
        ItemPost item = new ItemPost();
        try {
            item.setIdPost(json.getInt("id"));
            item.setSlug(json.getString("slug"));
            ...
            item.setOption(tabIndex);
            item.setCountview(json.getJSONObject("custom_fields").getJSONArray("post_views_count").get(0).toString());
    ...
    public static ArrayList<ItemPost> getVideoLocal(String tableName, String sql, int tabidex) {
        Cursor c = MainActivity.myDbHelper.query(tableName, null, null, null, null, null, null);
        c = MainActivity.myDbHelper.rawQuery(sql);
    ...
    public static boolean isExistFile(String urlFile) {
        File folder = new File(Environment.getExternalStorageDirectory() + "/VtcTube");
        if (!folder.exists()) {
            return false;
        }

        File myFile = new File(Environment.getExternalStorageDirectory() + urlFolder + urlFile);
        if (!myFile.exists()) {
            return false;
        }
    ...
    public static void writeJsonFile(String json, boolean isOverwriter, String pathFile) {
        try {
            File folder = new File(Environment.getExternalStorageDirectory() + "/VtcTube");
            if (!folder.exists()) {
                folder.mkdir();
            ...
            File myFile = new File(Environment.getExternalStorageDirectory() + urlFolder + pathFile);
            Log.d("path", Environment.getExternalStorageDirectory() + urlFolder + pathFile);
            if (!myFile.exists()) {
